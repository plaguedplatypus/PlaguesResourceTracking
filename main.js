...
var priceCacheDurationMs = 24 * 60 * 60 * 1000;
var sessionStatus = "idle";
...
var showGpValue = (_a = loadSessionSettings().showGpValue) !== null && _a !== void 0 ? _a : false;
var pendingPriceLookups = new Set();
function recordSessionUpdates(updates) {
...
function showSessionWindow() {
    if (!sessionWindow || sessionWindow.closed) {
        sessionWindow = window.open("", "sessionWindow", "width=400,height=275");
    }
...
function updateShowGpValue(value) {
    showGpValue = value;
    saveSessionSettings({
        showGpValue: showGpValue,
    });
    if (showGpValue) {
        void ensurePricesForSessionItems();
    }
...
function updateSessionWindow() {
    ...
    (_a = doc
        .getElementById("session-toggle")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", toggleSession);
    (_b = doc
        .getElementById("session-reset")) === null || _b === void 0 ? void 0 : _b.addEventListener("click", resetSession);
    var showGpInput = doc.getElementById("show-gp-value");
    if (showGpInput) {
...
function startSessionRefreshTimer() {
    if (sessionRefreshTimer !== null)
        return;
    sessionRefreshTimer = window.setInterval(function () {
        if (!sessionWindow || sessionWindow.closed) {
            if (sessionRefreshTimer !== null) {
...
function renderSessionWindowHtml() {
    var toggleText = sessionStatus === "idle"
        ? "Start Session"
        : sessionStatus === "running"
            ? "Pause Session"
            : "Resume Session";
    var startedText = sessionStartedAt
        ? new Date(sessionStartedAt).toLocaleTimeString("en-US", {
            hour12: false,
        })
        : "—";
    var statusText = sessionStatus === "idle"
        ? "Not running"
        : sessionStatus === "running"
            ? "Running"
            : "Paused";
...
function renderSessionItemsHtml() {
    var items = Object.keys(sessionItems).sort(function (a, b) {
        return sessionItems[b].lastUpdated - sessionItems[a].lastUpdated;
    });
    if (items.length === 0) {
    ...
    var header = showGpValue
        ? "\n\t\t\t<tr>\n\t\t\t\t<th class=\"item-name\">Item</th>\n\t\t\t\t<th class=\"number\">Count</th>\n\t\t\t\t<th class=\"number\">Per/hr</th>\n\t\t\t\t<th class=\"number\">Value</th>\n\t\t\t\t<th class=\"number\">GP/hr</th>\n\t\t\t</tr>\n\t\t"
        : "\n\t\t\t<tr>\n\t\t\t\t<th class=\"item-name\">Item</th>\n\t\t\t\t<th class=\"number\">Count</th>\n\t\t\t\t<th class=\"number\">/hr</th>\n\t\t\t</tr>\n\t\t";
    var rows = items
        .map(function (item) {
        var itemData = sessionItems[item];
        var perHour = elapsedHours > 0
            ? itemData.count / elapsedHours
            : 0;
        if (!showGpValue) {
            return "\n\t\t\t\t\t<tr>\n\t\t\t\t\t\t<td class=\"item-name\" title=\"".concat(escapeAttr(titleCase(item)), "\">\n\t\t\t\t\t\t\t").concat(escapeHtml(titleCase(item)), "\n\t\t\t\t\t\t</td>\n\t\t\t\t\t\t<td class=\"number\">").concat(itemData.count.toLocaleString(), "</td>\n\t\t\t\t\t\t<td class=\"number\">").concat(formatPerHour(perHour), "</td>\n\t\t\t\t\t</tr>\n\t\t\t\t");
        }
        var price = getFreshCachedPrice(item);
        var totalValue = typeof price === "number"
            ? itemData.count * price
            : null;
        var gpPerHour = totalValue !== null && elapsedHours > 0
            ? totalValue / elapsedHours
            : null;
        return "\n\t\t\t\t<tr>\n\t\t\t\t\t<td class=\"item-name\" title=\"".concat(escapeAttr(titleCase(item)), "\">\n\t\t\t\t\t\t").concat(escapeHtml(titleCase(item)), "\n\t\t\t\t\t</td>\n\t\t\t\t\t<td class=\"number\">").concat(itemData.count.toLocaleString(), "</td>\n\t\t\t\t\t<td class=\"number\">").concat(formatPerHour(perHour), "</td>\n\t\t\t\t\t<td class=\"number\">").concat(formatPriceValue(price, totalValue), "</td>\n\t\t\t\t\t<td class=\"number\">").concat(formatGpPerHour(gpPerHour), "</td>\n\t\t\t\t</tr>\n\t\t\t");
    })
        .join("");
...
function ensurePriceForItem(item) {
    return __awaiter(this, void 0, void 0, function () {
        var cacheKey, cachedPrice, price, cache, _a, cache;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    cacheKey = getItemCacheKey(item);
                    if (isCoinsItem(item))
                        return [2 /*return*/];
                    cachedPrice = getFreshCachedPrice(item);
                    if (cachedPrice !== undefined)
                        return [2 /*return*/];
                    if (pendingPriceLookups.has(cacheKey))
                        return [2 /*return*/];
                    pendingPriceLookups.add(cacheKey);
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 3, 4, 5]);
                    return [4 /*yield*/, fetchItemPrice(item)];
                case 2:
                    price = _b.sent();
...
function fetchItemPrice(item) {
    return __awaiter(this, void 0, void 0, function () {
        var wikiName, params, response, json, firstResult;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (isCoinsItem(item))
                        return [2 /*return*/, 1];
                    wikiName = toWikiPriceName(item);
                    params = new URLSearchParams({
                        name: wikiName,
                    });
                    return [4 /*yield*/, fetch("https://api.weirdgloop.org/exchange/history/rs/latest?".concat(params.toString()))];
                case 1:
                    response = _a.sent();
                    if (!response.ok)
                ...
                case 2:
                    json = _a.sent();
                    firstResult = Object.values(json)[0];
                    if (!firstResult || typeof firstResult.price !== "number") {
                        return [2 /*return*/, null];
                    }
                    return [2 /*return*/, firstResult.price];
            }
        });
    });
}
function getFreshCachedPrice(item) {
    if (isCoinsItem(item))
        return 1;
    var cache = loadPriceCache();
    var entry = cache[getItemCacheKey(item)];
    if (!entry)
        return undefined;
    var isFresh = Date.now() - entry.checkedAt < priceCacheDurationMs;
    if (!isFresh)
        return undefined;
    return entry.price;
}
function loadPriceCache() {
    var raw = localStorage.getItem(priceCacheKey);
    if (!raw)
        return {};
    try {
...
function loadSessionSettings() {
    var raw = localStorage.getItem(sessionSettingsKey);
    if (!raw)
        return {};
    try {
        return JSON.parse(raw);
...
function cleanItemNameForPrice(item) {
    return item
        .replace(/^﴾♦﴿\s*/, "")
        .toLowerCase()
        .replace(/\s+/g, " ")
...
function toWikiPriceName(item) {
    var cleaned = cleanItemNameForPrice(item);
    if (!cleaned)
        return "";
    var pageName = cleaned.charAt(0).toUpperCase() +
        cleaned.slice(1);
...
function formatGp(value) {
    ...
    if (rounded >= 1000000000) {
        return "".concat(trimDecimal(rounded / 1000000000), "b");
    }
    if (rounded >= 1000000) {
        return "".concat(trimDecimal(rounded / 1000000), "m");
    }
    if (rounded >= 10000) {
        return "".concat(trimDecimal(rounded / 1000), "k");
    }
...
(function webpackUniversalModuleDefinition(root, factory) {
	if(true)
		module.exports = factory((function webpackLoadOptionalExternalModule() { try { return __webpack_require__(/*! canvas */ "canvas"); } catch(e) {} }()), (function webpackLoadOptionalExternalModule() { try { return __webpack_require__(/*! electron/common */ "electron/common"); } catch(e) {} }()), (function webpackLoadOptionalExternalModule() { try { return __webpack_require__(/*! sharp */ "sharp"); } catch(e) {} }()));
	else // removed by dead control flow
{}
})(globalThis, (__WEBPACK_EXTERNAL_MODULE_canvas__, __WEBPACK_EXTERNAL_MODULE_electron_common__, __WEBPACK_EXTERNAL_MODULE_sharp__) => {
return /******/ (() => { // webpackBootstrap
/******/ 	"use strict";
/******/ 	var __webpack_modules__ = ({
...
(function () {
    ...
    if (!fillconstr) {
        exports.ImageData = globalThis.ImageData;
    }
    else {
        ...
        if (!filltype) {
            constr.prototype = globalThis.ImageData.prototype;
        }
        globalThis.ImageData = constr;
...
exports.ImageData.prototype.setPixel = function (x, y, rin, gin, bin, ain) {
    var [r, g, b, a] = (Array.isArray(rin) ? rin : [rin, gin, bin, ain]);
    var i = x * 4 + y * 4 * this.width;
    this.data[i] = r;
    this.data[i + 1] = g;
    this.data[i + 2] = b;
...
exports.ImageData.prototype.toFileBytes = function (format, quality) {
    if (typeof HTMLCanvasElement != "undefined") {
        return new Promise(d => this.toImage().toBlob(b => {
            ...
            r.onload = () => d(new Uint8Array(r.result).buffer);
        }, format, quality));
    }
    else {
...
/*!*********************************!*\
  !*** ./src/base/imagedetect.ts ***!
  \*********************************/
(__unused_webpack_module, exports, __nested_webpack_require_13079__) {
...
const imgref_1 = __nested_webpack_require_13079__(/*! ./imgref */ "./src/base/imgref.ts");
const wapper = __importStar(__nested_webpack_require_13079__(/*! ./wrapper */ "./src/base/wrapper.ts"));
const nodeimports = __importStar(__nested_webpack_require_13079__(/*! ./nodepolyfill */ "./src/base/nodepolyfill.ts"));
const _1 = __nested_webpack_require_13079__(/*! . */ "./src/base/index.ts");
/**
...
/*!****************************!*\
  !*** ./src/base/imgref.ts ***!
  \****************************/
(__unused_webpack_module, exports, __nested_webpack_require_30066__) {


Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ImgRefData = exports.ImgRefBind = exports.ImgRefCtx = exports.ImgRef = void 0;
const index_1 = __nested_webpack_require_30066__(/*! ./index */ "./src/base/index.ts");
/**
...
/*!***************************!*\
  !*** ./src/base/index.ts ***!
  \***************************/
(__unused_webpack_module, exports, __nested_webpack_require_34018__) {
...
__nested_webpack_require_34018__(/*! ./declarations */ "./src/base/declarations.ts");
exports.ImageDetect = __importStar(__nested_webpack_require_34018__(/*! ./imagedetect */ "./src/base/imagedetect.ts"));
exports.PasteInput = __importStar(__nested_webpack_require_34018__(/*! ./pasteinput */ "./src/base/pasteinput.ts"));
var rect_1 = __nested_webpack_require_34018__(/*! ./rect */ "./src/base/rect.ts");
Object.defineProperty(exports, "Rect", ({ enumerable: true, get: function () { return __importDefault(rect_1).default; } }));
var imagedata_extensions_1 = __nested_webpack_require_34018__(/*! ./imagedata-extensions */ "./src/base/imagedata-extensions.ts");
Object.defineProperty(exports, "ImageData", ({ enumerable: true, get: function () { return imagedata_extensions_1.ImageData; } }));
exports.NodePolyfill = __importStar(__nested_webpack_require_34018__(/*! ./nodepolyfill */ "./src/base/nodepolyfill.ts"));
__exportStar(__nested_webpack_require_34018__(/*! ./imgref */ "./src/base/imgref.ts"), exports);
__exportStar(__nested_webpack_require_34018__(/*! ./wrapper */ "./src/base/wrapper.ts"), exports);
var imagedetect_1 = __nested_webpack_require_34018__(/*! ./imagedetect */ "./src/base/imagedetect.ts");
...
/*!**********************************!*\
  !*** ./src/base/nodepolyfill.ts ***!
  \**********************************/
(__unused_webpack_module, exports, __nested_webpack_require_38153__) {
...
const index_1 = __nested_webpack_require_38153__(/*! ./index */ "./src/base/index.ts");
const imagedetect_1 = __nested_webpack_require_38153__(/*! ./imagedetect */ "./src/base/imagedetect.ts");
...
function requireSharp() {
    try {
        if (requirefunction) {
            ...
            return __nested_webpack_require_38153__(/* webpackIgnore: true */ /*! sharp */ "sharp"); // as typeof import("sharp");
        }
    }
    catch (e) { }
    return null;
...
function requireNodeCanvas() {
    ...
    try {
        if (requirefunction) {
            ...
            return __nested_webpack_require_38153__(/* webpackIgnore: true */ /*! canvas */ "canvas"); // as typeof import("sharp");
        }
    }
    catch (e) { }
    return null;
...
function requireElectronCommon() {
    try {
        if (requirefunction) {
            ...
            return __nested_webpack_require_38153__(/* webpackIgnore: true */ /*! electron/common */ "electron/common");
        }
    }
    catch (e) { }
    return null;
...
/*!********************************!*\
  !*** ./src/base/pasteinput.ts ***!
  \********************************/
(__unused_webpack_module, exports, __nested_webpack_require_44064__) {
...
const index_1 = __nested_webpack_require_44064__(/*! ./index */ "./src/base/index.ts");
const ImageDetect = __importStar(__nested_webpack_require_44064__(/*! ./imagedetect */ "./src/base/imagedetect.ts"));
var listeners = [];
var started = false;
...
/*!*****************************!*\
  !*** ./src/base/wrapper.ts ***!
  \*****************************/
(__unused_webpack_module, exports, __nested_webpack_require_55649__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ImageStreamReader = exports.skinName = exports.hasAlt1 = exports.newestversion = exports.Alt1Error = exports.NoAlt1Error = void 0;
...
const rect_1 = __importDefault(__nested_webpack_require_55649__(/*! ./rect */ "./src/base/rect.ts"));
const imgref_1 = __nested_webpack_require_55649__(/*! ./imgref */ "./src/base/imgref.ts");
const imagedata_extensions_1 = __nested_webpack_require_55649__(/*! ./imagedata-extensions */ "./src/base/imagedata-extensions.ts");
__nested_webpack_require_55649__(/*! ./alt1api */ "./src/base/alt1api.ts");
/**
...
/******/ 	function __nested_webpack_require_77795__(moduleId) {
...
/******/ 			throw e;
/******/ 		}
/******/ 		__webpack_modules__[moduleId].call(module.exports, module, module.exports, __nested_webpack_require_77795__);
/******/ 	
...
/******/ 	var __nested_webpack_exports__ = __nested_webpack_require_77795__("./src/base/index.ts");
/******/ 	
...
/*!***********************************************************!*\
  !*** ./src/chatbox/imgs/badges/broadcast_bronze.data.png ***!
  \***********************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_941__) {

module.exports=(__nested_webpack_require_941__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*********************************************************!*\
  !*** ./src/chatbox/imgs/badges/broadcast_gold.data.png ***!
  \*********************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_2403__) {

module.exports=(__nested_webpack_require_2403__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************************!*\
  !*** ./src/chatbox/imgs/badges/broadcast_silver.data.png ***!
  \***********************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_6710__) {

module.exports=(__nested_webpack_require_6710__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/badges/gim.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_10969__) {

module.exports=(__nested_webpack_require_10969__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/badges/hcim.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_11660__) {

module.exports=(__nested_webpack_require_11660__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**************************************************!*\
  !*** ./src/chatbox/imgs/badges/ironman.data.png ***!
  \**************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_12343__) {

module.exports=(__nested_webpack_require_12343__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!******************************************************!*\
  !*** ./src/chatbox/imgs/badges/league_addy.data.png ***!
  \******************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_13050__) {

module.exports=(__nested_webpack_require_13050__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!********************************************************!*\
  !*** ./src/chatbox/imgs/badges/league_dragon.data.png ***!
  \********************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_13785__) {

module.exports=(__nested_webpack_require_13785__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!******************************************************!*\
  !*** ./src/chatbox/imgs/badges/league_mith.data.png ***!
  \******************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_14524__) {

module.exports=(__nested_webpack_require_14524__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!******************************************************!*\
  !*** ./src/chatbox/imgs/badges/league_rune.data.png ***!
  \******************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_15263__) {

module.exports=(__nested_webpack_require_15263__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*******************************************************!*\
  !*** ./src/chatbox/imgs/badges/league_steel.data.png ***!
  \*******************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_15994__) {

module.exports=(__nested_webpack_require_15994__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/badges/link.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_16705__) {

module.exports=(__nested_webpack_require_16705__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/badges/pmod.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_17416__) {

module.exports=(__nested_webpack_require_17416__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**************************************************!*\
  !*** ./src/chatbox/imgs/badges/pmodvip.data.png ***!
  \**************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_18079__) {

module.exports=(__nested_webpack_require_18079__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/badges/rgim.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_18734__) {

module.exports=(__nested_webpack_require_18734__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/badges/vip.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_19477__) {

module.exports=(__nested_webpack_require_19477__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!****************************************************!*\
  !*** ./src/chatbox/imgs/chatLegacyBorder.data.png ***!
  \****************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_20144__) {

module.exports=(__nested_webpack_require_20144__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("iVBORw0KGgoAAAANSUhEUgAAAAQAAAAPCAIAAABMVPnqAAAAIUlEQVQYV2P4jwRAnNk92RA0mDjzJuRCOUAWlAMF//8DAJmOfBu3Ydq7AAAAAElFTkSuQmCC")
...
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/chatbubble.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_20639__) {

module.exports=(__nested_webpack_require_20639__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/clanchat.data.png"
/*!********************************************!*\
  !*** ./src/chatbox/imgs/clanchat.data.png ***!
  \********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_21282__) {

module.exports=(__nested_webpack_require_21282__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!************************************************!*\
  !*** ./src/chatbox/imgs/filterbutton.data.png ***!
  \************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_22604__) {

module.exports=(__nested_webpack_require_22604__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/friendschat.data.png"
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/friendschat.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_23275__) {

module.exports=(__nested_webpack_require_23275__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*******************************************!*\
  !*** ./src/chatbox/imgs/gameall.data.png ***!
  \*******************************************/
(module, __unused_webpack_exports, __nested_webpack_require_23954__) {

module.exports=(__nested_webpack_require_23954__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/gamefilter.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_24681__) {

module.exports=(__nested_webpack_require_24681__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*******************************************!*\
  !*** ./src/chatbox/imgs/gameoff.data.png ***!
  \*******************************************/
(module, __unused_webpack_exports, __nested_webpack_require_25444__) {

module.exports=(__nested_webpack_require_25444__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/gimchat.data.png"
/*!*******************************************!*\
  !*** ./src/chatbox/imgs/gimchat.data.png ***!
  \*******************************************/
(module, __unused_webpack_exports, __nested_webpack_require_26083__) {

module.exports=(__nested_webpack_require_26083__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/groupchat.data.png"
/*!*********************************************!*\
  !*** ./src/chatbox/imgs/groupchat.data.png ***!
  \*********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_26902__) {

module.exports=(__nested_webpack_require_26902__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*********************************************!*\
  !*** ./src/chatbox/imgs/guestclan.data.png ***!
  \*********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_27601__) {

module.exports=(__nested_webpack_require_27601__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!************************************************!*\
  !*** ./src/chatbox/imgs/legacyreport.data.png ***!
  \************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_28292__) {

module.exports=(__nested_webpack_require_28292__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/minusbutton.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_28947__) {

module.exports=(__nested_webpack_require_28947__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/plusbutton.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_29834__) {

module.exports=(__nested_webpack_require_29834__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*************************************************!*\
  !*** ./src/chatbox/imgs/privateRecent.data.png ***!
  \*************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_30741__) {

module.exports=(__nested_webpack_require_30741__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/privatechat.data.png"
/*!***********************************************!*\
  !*** ./src/chatbox/imgs/privatechat.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_31428__) {

module.exports=(__nested_webpack_require_31428__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")

/***/ },

/***/ "./src/chatbox/imgs/publicchat.data.png"
/*!**********************************************!*\
  !*** ./src/chatbox/imgs/publicchat.data.png ***!
  \**********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_32123__) {

module.exports=(__nested_webpack_require_32123__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!************************************************!*\
  !*** ./src/chatbox/imgs/reportbutton.data.png ***!
  \************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_32790__) {

module.exports=(__nested_webpack_require_32790__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!******************************!*\
  !*** ./src/chatbox/index.ts ***!
  \******************************/
(__unused_webpack_module, exports, __nested_webpack_require_33449__) {
...
const a1lib = __importStar(__nested_webpack_require_33449__(/*! alt1/base */ "alt1/base"));
const base_1 = __nested_webpack_require_33449__(/*! alt1/base */ "alt1/base");
const OCR = __importStar(__nested_webpack_require_33449__(/*! alt1/ocr */ "alt1/ocr"));
const base_2 = __nested_webpack_require_33449__(/*! alt1/base */ "alt1/base");
let chatfont = __nested_webpack_require_33449__(/*! ../fonts/aa_8px.fontmeta.json */ "./src/fonts/aa_8px.fontmeta.json");
let fonts = [
    { name: "10pt", lineheight: 14, badgey: -9, dy: -2, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/10pt.fontmeta.json */ "./src/fonts/chatbox/10pt.fontmeta.json") },
    { name: "12pt", lineheight: 16, badgey: -9, dy: -3, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/12pt.fontmeta.json */ "./src/fonts/chatbox/12pt.fontmeta.json") },
    { name: "14pt", lineheight: 18, badgey: -10, dy: -3, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/14pt.fontmeta.json */ "./src/fonts/chatbox/14pt.fontmeta.json") },
    { name: "16pt", lineheight: 21, badgey: -10, dy: -4, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/16pt.fontmeta.json */ "./src/fonts/chatbox/16pt.fontmeta.json") },
    { name: "18pt", lineheight: 23, badgey: -11, dy: -4, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/18pt.fontmeta.json */ "./src/fonts/chatbox/18pt.fontmeta.json") },
    { name: "20pt", lineheight: 25, badgey: -11, dy: -5, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/20pt.fontmeta.json */ "./src/fonts/chatbox/20pt.fontmeta.json") },
    { name: "22pt", lineheight: 27, badgey: -12, dy: -5, def: __nested_webpack_require_33449__(/*! ../fonts/chatbox/22pt.fontmeta.json */ "./src/fonts/chatbox/22pt.fontmeta.json") },
];
const imgs = (0, base_2.webpackImages)({
    plusbutton: __nested_webpack_require_33449__(/*! ./imgs/plusbutton.data.png */ "./src/chatbox/imgs/plusbutton.data.png"),
    minusbutton: __nested_webpack_require_33449__(/*! ./imgs/minusbutton.data.png */ "./src/chatbox/imgs/minusbutton.data.png"),
    filterbutton: __nested_webpack_require_33449__(/*! ./imgs/filterbutton.data.png */ "./src/chatbox/imgs/filterbutton.data.png"),
    chatbubble: __nested_webpack_require_33449__(/*! ./imgs/chatbubble.data.png */ "./src/chatbox/imgs/chatbubble.data.png"),
    chatLegacyBorder: __nested_webpack_require_33449__(/*! ./imgs/chatLegacyBorder.data.png */ "./src/chatbox/imgs/chatLegacyBorder.data.png"),
    entertochat: __nested_webpack_require_33449__(/*! ./imgs/entertochat.data.png */ "./src/chatbox/imgs/entertochat.data.png"),
    gameoff: __nested_webpack_require_33449__(/*! ./imgs/gameoff.data.png */ "./src/chatbox/imgs/gameoff.data.png"),
    gamefilter: __nested_webpack_require_33449__(/*! ./imgs/gamefilter.data.png */ "./src/chatbox/imgs/gamefilter.data.png"),
    gameall: __nested_webpack_require_33449__(/*! ./imgs/gameall.data.png */ "./src/chatbox/imgs/gameall.data.png"),
    legacyreport: __nested_webpack_require_33449__(/*! ./imgs/legacyreport.data.png */ "./src/chatbox/imgs/legacyreport.data.png"),
    reportbutton: __nested_webpack_require_33449__(/*! ./imgs/reportbutton.data.png */ "./src/chatbox/imgs/reportbutton.data.png"),
});
const chatimgs = (0, base_2.webpackImages)({
    public: __nested_webpack_require_33449__(/*! ./imgs/publicchat.data.png */ "./src/chatbox/imgs/publicchat.data.png"),
    private: __nested_webpack_require_33449__(/*! ./imgs/privatechat.data.png */ "./src/chatbox/imgs/privatechat.data.png"),
    clan: __nested_webpack_require_33449__(/*! ./imgs/clanchat.data.png */ "./src/chatbox/imgs/clanchat.data.png"),
    guestclan: __nested_webpack_require_33449__(/*! ./imgs/guestclan.data.png */ "./src/chatbox/imgs/guestclan.data.png"),
    privateRecent: __nested_webpack_require_33449__(/*! ./imgs/privateRecent.data.png */ "./src/chatbox/imgs/privateRecent.data.png"),
    friends: __nested_webpack_require_33449__(/*! ./imgs/friendschat.data.png */ "./src/chatbox/imgs/friendschat.data.png"),
    group: __nested_webpack_require_33449__(/*! ./imgs/groupchat.data.png */ "./src/chatbox/imgs/groupchat.data.png"),
    groupironman: __nested_webpack_require_33449__(/*! ./imgs/gimchat.data.png */ "./src/chatbox/imgs/gimchat.data.png"),
});
const chatmap = {
    public: "main",
    private: "private",
...
const chatbadges = (0, base_2.webpackImages)({
    vip: __nested_webpack_require_33449__(/*! ./imgs/badges/vip.data.png */ "./src/chatbox/imgs/badges/vip.data.png"),
    pmod: __nested_webpack_require_33449__(/*! ./imgs/badges/pmod.data.png */ "./src/chatbox/imgs/badges/pmod.data.png"),
    pmodvip: __nested_webpack_require_33449__(/*! ./imgs/badges/pmodvip.data.png */ "./src/chatbox/imgs/badges/pmodvip.data.png"),
    broadcast_gold: __nested_webpack_require_33449__(/*! ./imgs/badges/broadcast_gold.data.png */ "./src/chatbox/imgs/badges/broadcast_gold.data.png"),
    broadcast_silver: __nested_webpack_require_33449__(/*! ./imgs/badges/broadcast_silver.data.png */ "./src/chatbox/imgs/badges/broadcast_silver.data.png"),
    broadcast_bronze: __nested_webpack_require_33449__(/*! ./imgs/badges/broadcast_bronze.data.png */ "./src/chatbox/imgs/badges/broadcast_bronze.data.png"),
    broadcast_death: __nested_webpack_require_33449__(/*! ./imgs/badges/broadcast_death.data.png */ "./src/chatbox/imgs/badges/broadcast_death.data.png"),
    ironman: __nested_webpack_require_33449__(/*! ./imgs/badges/ironman.data.png */ "./src/chatbox/imgs/badges/ironman.data.png"),
    hcim: __nested_webpack_require_33449__(/*! ./imgs/badges/hcim.data.png */ "./src/chatbox/imgs/badges/hcim.data.png"),
    rgim: __nested_webpack_require_33449__(/*! ./imgs/badges/rgim.data.png */ "./src/chatbox/imgs/badges/rgim.data.png"),
    gim: __nested_webpack_require_33449__(/*! ./imgs/badges/gim.data.png */ "./src/chatbox/imgs/badges/gim.data.png"),
    chatlink: __nested_webpack_require_33449__(/*! ./imgs/badges/link.data.png */ "./src/chatbox/imgs/badges/link.data.png"),
    league_steel: __nested_webpack_require_33449__(/*! ./imgs/badges/league_steel.data.png */ "./src/chatbox/imgs/badges/league_steel.data.png"),
    league_mith: __nested_webpack_require_33449__(/*! ./imgs/badges/league_mith.data.png */ "./src/chatbox/imgs/badges/league_mith.data.png"),
    league_addy: __nested_webpack_require_33449__(/*! ./imgs/badges/league_addy.data.png */ "./src/chatbox/imgs/badges/league_addy.data.png"),
    league_rune: __nested_webpack_require_33449__(/*! ./imgs/badges/league_rune.data.png */ "./src/chatbox/imgs/badges/league_rune.data.png"),
    league_dragon: __nested_webpack_require_33449__(/*! ./imgs/badges/league_dragon.data.png */ "./src/chatbox/imgs/badges/league_dragon.data.png"),
});
const badgemap = {
...
/******/ 	
/******/ 	// The require function
/******/ 	function __nested_webpack_require_521559__(moduleId) {
/******/ 		// Check if module is in cache
/******/ 		var cachedModule = __webpack_module_cache__[moduleId];
/******/ 		if (cachedModule !== undefined) {
/******/ 			return cachedModule.exports;
...
/******/ 			throw e;
/******/ 		}
/******/ 		__webpack_modules__[moduleId].call(module.exports, module, module.exports, __nested_webpack_require_521559__);
/******/ 	
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
...
/******/ 	// Load entry module and return exports
/******/ 	// This entry module is referenced by other modules so it can't be inlined
/******/ 	var __nested_webpack_exports__ = __nested_webpack_require_521559__("./src/chatbox/index.ts");
/******/ 	
/******/ 	return __nested_webpack_exports__;
/******/ })()
;
...
/*!******************************************!*\
  !*** ./src/dialog/imgs/chatimg.data.png ***!
  \******************************************/
(module, __unused_webpack_exports, __nested_webpack_require_4763__) {

module.exports=(__nested_webpack_require_4763__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!************************************************!*\
  !*** ./src/dialog/imgs/chatimgactive.data.png ***!
  \************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_6397__) {

module.exports=(__nested_webpack_require_6397__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!****************************************************!*\
  !*** ./src/dialog/imgs/chatimgactive_leg.data.png ***!
  \****************************************************/
(module, __unused_webpack_exports, __nested_webpack_require_7164__) {

module.exports=(__nested_webpack_require_7164__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!***********************************************!*\
  !*** ./src/dialog/imgs/chatimghover.data.png ***!
  \***********************************************/
(module, __unused_webpack_exports, __nested_webpack_require_8035__) {

module.exports=(__nested_webpack_require_8035__(/*! alt1/base */ "alt1/base").ImageDetect).imageDataFromBase64("")
...
/*!*****************************!*\
  !*** ./src/dialog/index.ts ***!
  \*****************************/
(__unused_webpack_module, exports, __nested_webpack_require_16005__) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const a1lib = __importStar(__nested_webpack_require_16005__(/*! alt1/base */ "alt1/base"));
const OCR = __importStar(__nested_webpack_require_16005__(/*! alt1/ocr */ "alt1/ocr"));
const base_1 = __nested_webpack_require_16005__(/*! alt1/base */ "alt1/base");
var imgs_rs3 = (0, base_1.webpackImages)({
    chatimg: __nested_webpack_require_16005__(/*! ./imgs/chatimg.data.png */ "./src/dialog/imgs/chatimg.data.png"),
    chatimghover: __nested_webpack_require_16005__(/*! ./imgs/chatimghover.data.png */ "./src/dialog/imgs/chatimghover.data.png"),
    chatimgactive: __nested_webpack_require_16005__(/*! ./imgs/chatimgactive.data.png */ "./src/dialog/imgs/chatimgactive.data.png"),
    continueimg: __nested_webpack_require_16005__(/*! ./imgs/continueimg.data.png */ "./src/dialog/imgs/continueimg.data.png"),
    continueimgdown: __nested_webpack_require_16005__(/*! ./imgs/continueimgdown.data.png */ "./src/dialog/imgs/continueimgdown.data.png"),
    boxtl: __nested_webpack_require_16005__(/*! ./imgs/boxtl.data.png */ "./src/dialog/imgs/boxtl.data.png"),
    boxtr: __nested_webpack_require_16005__(/*! ./imgs/boxtr.data.png */ "./src/dialog/imgs/boxtr.data.png")
});
var imgs_leg = (0, base_1.webpackImages)({
    chatimg: __nested_webpack_require_16005__(/*! ./imgs/chatimg_leg.data.png */ "./src/dialog/imgs/chatimg_leg.data.png"),
    chatimghover: __nested_webpack_require_16005__(/*! ./imgs/chatimghover_leg.data.png */ "./src/dialog/imgs/chatimghover_leg.data.png"),
    chatimgactive: __nested_webpack_require_16005__(/*! ./imgs/chatimgactive_leg.data.png */ "./src/dialog/imgs/chatimgactive_leg.data.png"),
    continueimg: __nested_webpack_require_16005__(/*! ./imgs/continueimg_leg.data.png */ "./src/dialog/imgs/continueimg_leg.data.png"),
    continueimgdown: __nested_webpack_require_16005__(/*! ./imgs/continueimgdown_leg.data.png */ "./src/dialog/imgs/continueimgdown_leg.data.png"),
    boxtl: __nested_webpack_require_16005__(/*! ./imgs/boxtl_leg.data.png */ "./src/dialog/imgs/boxtl_leg.data.png"),
    boxtr: __nested_webpack_require_16005__(/*! ./imgs/boxtr_leg.data.png */ "./src/dialog/imgs/boxtr_leg.data.png")
});
var fontmono = __nested_webpack_require_16005__(/*! ../fonts/aa_8px_mono.fontmeta.json */ "./src/fonts/aa_8px_mono.fontmeta.json");
var fontmono2 = __nested_webpack_require_16005__(/*! ./imgs/12pt.fontmeta.json */ "./src/dialog/imgs/12pt.fontmeta.json");
var fontheavy = __nested_webpack_require_16005__(/*! ../fonts/aa_8px_mono_allcaps.fontmeta.json */ "./src/fonts/aa_8px_mono_allcaps.fontmeta.json");
class DialogReader {
    constructor() {
        this.pos = null;
    }
...
/******/ 	
/******/ 	// The require function
/******/ 	function __nested_webpack_require_86629__(moduleId) {
/******/ 		// Check if module is in cache
/******/ 		var cachedModule = __webpack_module_cache__[moduleId];
/******/ 		if (cachedModule !== undefined) {
/******/ 			return cachedModule.exports;
...
/******/ 			throw e;
/******/ 		}
/******/ 		__webpack_modules__[moduleId].call(module.exports, module, module.exports, __nested_webpack_require_86629__);
/******/ 	
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
...
/******/ 	// Load entry module and return exports
/******/ 	// This entry module is referenced by other modules so it can't be inlined
/******/ 	var __nested_webpack_exports__ = __nested_webpack_require_86629__("./src/dialog/index.ts");
/******/ 	
/******/ 	return __nested_webpack_exports__;
/******/ })()
;
...
var findChatButton = document.querySelector(".find-chat");
...
function populateChatSelector() {
    ...
    chatSelector.onchange = function () {
        ...
        saveData(data);
        status.innerText = "Using Chat ".concat(chatSelector.value, ".");
    };
}
...
function incrementItems(updates, highlightItem) {
    ...
    try {
        (0,_session__WEBPACK_IMPORTED_MODULE_4__.recordSessionUpdates)(updates);
    }
    catch (error) {
...
function render(highlightItem, data) {
    ...
    if (activeSkillTab === "archaeology") {
        ...
        var artefacts = items.filter(function (item) {
            return isDamagedArtefact(item);
        });
...
function refreshChatboxes() {
    ...
    var data = getSaveData();
    var savedChat = data.chat || "0";
    var selectedChat = chatSelector.value || savedChat;
    var selectedIndex = Number(selectedChat);
    var validIndex = found.boxes[selectedIndex] ? selectedIndex : 0;
    var validChat = String(validIndex);
    found.mainbox = found.boxes[validIndex];
    chatSelector.value = validChat;
    data.chat = validChat;
    saveData(data);
    showSelectedChat(found);
    status.innerText = "Chatbox refreshed. Using Chat ".concat(validChat, ".");
}
